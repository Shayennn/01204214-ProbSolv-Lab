...
{
    drink1->nexts.erase(remove(drink1->nexts.begin(), drink1->nexts.end(), drink2),drink1->nexts.end());
    drink2->previous.erase(remove(drink2->previous.begin(), drink2->previous.end(), drink1),drink2->previous.end());
}
...
void processLink(Drink *&drink1, Drink *&drink2)
{
    ...
    {
        vector<Drink *>::iterator i;
        for (i = drink1->nexts.begin();i != drink1->nexts.end();++i)
        {
        ...
        if (i == drink1->nexts.end()) drink1->nexts.insert(i, drink2);
    }
}
...
void topIterate(vector<Drink *> &drinks)
{
    ...
    while (!drinkDeque.empty())
    {
        ...
        for (int k = 0; k < nexts.size(); ++k) removeLink(drink, nexts[k]);
        Drink *curDink = drinking(drinks);
        if (curDink == NULL) break;
...
{
    vector<Drink *> drinks;
    for (int j = 0; j < keywords.size() - 1; ++j) process1(drinks, keywords[j], keywords[j + 1]);
    topIterate(drinks);
    cout << endl;
}

...
int main()
{
    ...
    while (cin >> R >> C >> S, R)
    {
        ...
        for (int r = 0; r < R; ++r)
        {
            ...
            for (int c = 0; c < C; ++c)
            {
                if (line[c] == '#') passable[r][c] = false;
                else
                {
                    passable[r][c] = true;
                    if (line[c] == '*') hasSticker[r][c] = true;
                    else
                    {
        ...
        for (int i = 0; i < S; ++i)
        {
            if (line[i] == 'D') orientation = (orientation - 1 + NumChanges) % NumChanges;
            else if (line[i] == 'E') orientation = (orientation + 1) % NumChanges;
            else
            {
